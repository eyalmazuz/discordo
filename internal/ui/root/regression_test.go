...
func TestRegression_AppLifecycle(t *testing.T) {
	cfg, _ := config.Load("")
	app := tview.NewApplication(tview.WithScreen(&mockScreen{}))
	...
	app.Send(&token.TokenMsg{Token: testToken})
	time.Sleep(200 * time.Millisecond)
...
func TestRegression_GlobalKeybinds(t *testing.T) {
	...
	app.Send(tcell.NewEventKey(tcell.KeyRune, ".", tcell.ModCtrl))
	time.Sleep(200 * time.Millisecond)
