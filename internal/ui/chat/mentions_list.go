...
func (m *mentionsList) View(screen tcell.Screen) {
...
func (m *mentionsList) previewItemFor(item mentionsListItem) *imageItem {
	if !m.cfg.InlineImages.Enabled || item.previewURL == "" {
		return nil
	}
	key := "mention:" + item.previewURL
	if existing, ok := m.emoteItemByKey[key]; ok {
		return existing
	}
	imgItem := newImageItem(m.imageCache, item.previewURL, inlineEmoteWidth, 1, m.useKitty, m.nextKittyID, nil, nil)
	m.nextKittyID++
...
func (i *mentionsListRowItem) Height(width int) int { return 1 }

func (i *mentionsListRowItem) View(screen tcell.Screen) {
	x, y, w, h := i.InnerRect()
	if w <= 0 || h <= 0 {
		return
	}
